...
QWidget *OBSPropertiesView::NewWidget(obs_property_t *prop, QWidget *new_widget,
				      const char *signal)
{
	const char *long_desc = obs_property_long_description(prop);

	WidgetInfo *info = new WidgetInfo(this, prop, new_widget);
	connect(new_widget, signal, info, SLOT(ControlChanged()));
	children.emplace_back(info);

	new_widget->setToolTip(QT_UTF8(long_desc));
	return new_widget;
}
...
void OBSPropertiesView::AddInt(obs_property_t *prop, QFormLayout *layout,
			       QLabel **label)
{
	obs_number_type int_type = obs_property_int_type(prop);
	QLayout *subLayout = new QHBoxLayout();
...
void OBSPropertiesView::AddFloat(obs_property_t *prop, QFormLayout *layout,
				 QLabel **label)
{
	obs_number_type float_type = obs_property_float_type(prop);
	QLayout *subLayout = new QHBoxLayout();
...
QWidget *OBSPropertiesView::AddList(obs_property_t *prop, bool &warning)
{
	...
	obs_combo_type list_type = obs_property_list_type(prop);
				     ...
				     QFormLayout *layout, QLabel *&label)
{
	...
	auto wdgt = CreateFrameRateWidget(prop, warning, option, valid_fps,
					    fps_ranges);
	auto info = new WidgetInfo(this, prop, wdgt);

	wdgt->setToolTip(QT_UTF8(obs_property_long_description(prop)));

	wdgt->name = name;
	wdgt->settings = settings;

	wdgt->modeSelect->setEnabled(enabled);
	wdgt->simpleFPS->setEnabled(enabled);
	wdgt->fpsRange->setEnabled(enabled);
	wdgt->numEdit->setEnabled(enabled);
	wdgt->denEdit->setEnabled(enabled);

	label = wdgt->warningLabel =
		new QLabel{obs_property_description(prop)};

	layout->addRow(label, wdgt);

	children.emplace_back(info);

	UpdateFPSLabels(wdgt);

	auto stack = wdgt->modeDisplay;
	auto combo = wdgt->modeSelect;
	...
	connect(wdgt->simpleFPS, comboIndexChanged, [=](int) {
		if (wdgt->updating)
			return;
	...
	connect(wdgt->fpsRange, comboIndexChanged, [=](int) {
		if (wdgt->updating)
			return;
	...
	connect(wdgt->numEdit, sbValueChanged, [=](int) {
		if (wdgt->updating)
			return;
	...
	connect(wdgt->denEdit, sbValueChanged, [=](int) {
		if (wdgt->updating)
			return;
				    ...
				    QFormLayout *layout)
{
	...
	case OBS_PROPERTY_BOOL:
		add_widget = AddCheckbox(property);
		break;
	...
	case OBS_PROPERTY_TEXT:
		add_widget = AddText(property, layout, label);
		break;
	case OBS_PROPERTY_PATH:
		AddPath(property, layout, &label);
		break;
	case OBS_PROPERTY_LIST:
		add_widget = AddList(property, warning);
		break;
	...
	if (!label && prop_type != OBS_PROPERTY_BOOL &&
	    prop_type != OBS_PROPERTY_BUTTON && prop_type != OBS_PROPERTY_GROUP)
	...
	if (obs_property_long_description(property)) {
		...
		if (label) {
		...
		} else if (prop_type == OBS_PROPERTY_BOOL) {
			...
			QCheckBox *check =
				qobject_cast<QCheckBox *>(add_widget);
			check->setText(desc);
			check->setToolTip(
				obs_property_long_description(property));
	...
	layout->addRow(label, add_widget);

	if (!lastFocused.empty())
		if (lastFocused.compare(name) == 0)
			lastWidget = add_widget;
}
